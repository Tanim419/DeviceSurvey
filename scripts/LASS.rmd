...
plot_dir <- file.path(project_directory, 'Plots/LASS')
...
write.csv(Device_coreQ,file.path(curated_data_dir, file_name), row.names = FALSE)
 levels(factor(Device_coreQ$Department))
```
...
#####Test a single core questions
  ...
  temp <- count(Device_coreQ$Department )
...
# 
temp["Rest", ] <- temp["2",] + temp["6",] + temp["9",] + temp["10",]+temp["13",]+temp["14",]
    temp <- temp[rownames(temp) != "2",]
    temp <- temp[rownames(temp) != "6",]
    temp <- temp[rownames(temp) != "9",]
    temp <- temp[rownames(temp) != "10",]
    temp <- temp[rownames(temp) != "13",]
    temp <- temp[rownames(temp) != "14",]
# 
    temp$item <-
...
for (i in 1:length(Device_coreQ)) {
  ...
  bar_plot <- ggplot(data = temp, aes(x = item, y = cou)) +
    geom_bar(stat = "identity",
             ...
             fill = "steelblue") +
    theme_bw() +
    scale_y_continuous(breaks = seq(0, ylimit, by = 106),
                       limits = c(0, ylimit)) +
  ...
  if (i == 1) {
    ...
    #
    temp["Rest",] <-
      temp["2", ] + temp["6", ] + temp["9", ] + temp["10", ] + temp["13", ] + temp["14", ]
    temp <- temp[rownames(temp) != "2", ]
    temp <- temp[rownames(temp) != "6", ]
    temp <- temp[rownames(temp) != "9", ]
    temp <- temp[rownames(temp) != "10", ]
