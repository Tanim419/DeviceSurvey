...
plot_dir <- file.path(project_directory, 'Plots/NSM_Staff')
...
# Device_data<- subset(Device_data, Device_data$College=="College of Liberal Arts and Social Sciences")
Device_data<- subset(Device_data, Device_data$College=="College of Natural Sciences and Mathematics")
...
for (i in 1:nrow(Device_data)) {
  ...
  if (Device_data$Main.department.1[i]=="Communication Sciences and Disorders"){
    Device_data$Main.department.1[i]="Communication Disorders"
  }
  if (Device_data$Main.department.1[i]=="Jack J. Valenti School of Communication"){
    Device_data$Main.department.1[i]="School of Communication"
  }
...
write.csv(Device_coreQ,file.path(curated_data_dir, file_name), row.names = FALSE)
 # levels(factor(Device_coreQ$classification))
levels(factor(Device_coreQ$Smartphone_model))

Device_coreQ<-subset(Device_coreQ, Classification !="Freshman" &
                       Classification !="Junior" &
                       Classification !="MS" &
                       Classification !="PhD" &
                       Classification !="Senior" &
                       Classification !="Sophomore" &
                       Classification !="P-B non-degree" &
                       Classification !="Postdoctoral" &
...
ownership = c("Own", "University property")
os = c("Mac OS",  "Windows OS",   "Other")
os_version = c(
  "Catalina",
  "High Sierra",
  "Mojave",
...
sp_model = c("iPhone 8", "iPhone X Series", "iPhone 11", "Google Pixel 3a", "Samsung Galaxy", "Other")
...
#####Test a single core questions
  ...
  temp <- count(Device_coreQ$Classification)
...
# # 
# temp["Rest", ] <- temp["1",] + temp["4",] + temp["5",] + temp["7",]
...
for (i in 1:length(order_list)) {
  ...
  bar_plot <- ggplot(data = temp, aes(x = item, y = cou)) +
    geom_bar(stat = "identity",
             ...
             fill = "steelblue") +
    theme_bw() +
    scale_y_continuous(breaks = seq(0, ylimit, by = 21),
                       limits = c(0, ylimit)) +
 ...
 if (i == 1) {
    ...
    temp <- count(Device_coreQ$Race)
    colnames(temp) <- c("item", "cou")
    temp <- temp[!(temp$item == ""), ]
    temp <- temp[order(temp$cou),]
    # temp$order<-order_list[[i]]
    temp <- temp[match(order_list[[4]], temp$item), ]
    temp <- temp %>% mutate(item = factor(x = item, levels = item))
    #
    temp["Rest",] <- temp["1", ] + temp["4", ] + temp["5", ] + temp["7", ]
    temp <- temp[rownames(temp) != "1", ]
    ...
    temp["Dell Inspiration", ] <-temp["3",] + temp["4",] + temp["5",]
